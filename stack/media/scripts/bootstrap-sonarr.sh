#!/bin/bash
# Bootstrap Sonarr - Configure authentication, download client, and root folder
#
# This script:
# 1. Waits for Sonarr to be ready
# 2. Extracts API key from config.xml
# 3. Enables Forms authentication via API
# 4. Adds qBittorrent download client
# 5. Adds TV root folder
...
configure_auth() {
    ...
    current_auth=$(curl -ks "$SONARR_URL/api/v3/config/host" \
        -H "X-Api-Key: $api_key" | jq -r '.authenticationMethod // "None"')

    if [ "$current_auth" = "Forms" ]; then
        log_success "Sonarr already configured with Forms authentication"
        return 0
    fi

    log_info "Configuring Forms authentication..."

    config=$(curl -ks "$SONARR_URL/api/v3/config/host" -H "X-Api-Key: $api_key")
    ...
    result=$(curl -ks -X PUT "$SONARR_URL/api/v3/config/host" \
        -H "X-Api-Key: $api_key" \
    ...
    if [ "$result" = "202" ] || [ "$result" = "200" ]; then
        log_success "Sonarr Forms authentication enabled"
    else
...
add_download_client() {
    ...
    existing=$(curl -ks "$SONARR_URL/api/v3/downloadclient" \
...
EOF
    ...
    result=$(curl -ks -X POST "$SONARR_URL/api/v3/downloadclient" \
        -H "X-Api-Key: $api_key" \
        -H "Content-Type: application/json" \
        -d "$client_config" \
        -o /dev/null -w "%{http_code}")
...
add_root_folder() {
    local api_key="$1"
    local path="/media/library/tv"

    # Check if root folder already exists
    existing=$(curl -ks "$SONARR_URL/api/v3/rootfolder" \
    ...
    result=$(curl -ks -X POST "$SONARR_URL/api/v3/rootfolder" \
        -H "X-Api-Key: $api_key" \
        -H "Content-Type: application/json" \
        -d "{\"path\": \"$path\"}" \
        -o /dev/null -w "%{http_code}")
...
main() {
    log_info "=== Bootstrapping Sonarr ==="

    # Wait for Sonarr to be ready
    wait_for_service "Sonarr" "$SONARR_URL" || exit 1

    # Get API key (from .env or extract from config)
    api_key=$(get_env "SONARR_API_KEY")
    if [ -z "$api_key" ]; then
        log_info "Extracting API key from config..."
        api_key=$(get_arr_api_key "sonarr")
        if [ -z "$api_key" ]; then
            log_error "Could not extract Sonarr API key"
            exit 1
        fi
        update_env "SONARR_API_KEY" "$api_key"
    else
    ...
    log_success "Sonarr bootstrap complete"
